...
(function () {
  ...
  function readFlatPreconfig(obj) {
    ...
    readFlatPreconfigBoolean(
      obj.qunit_config_failonzerotests,
      "failOnZeroTests"
    );
      ...
      },
      {
        ...
        value: function closeTo(actual, expected, delta, message) {
          ...
          this.pushResult({
            ...
            expected: expected,
            message:
              message || "value should be within ".concat(delta, " inclusive"),
          });
        },
    ...
    finish: function finish() {
      ...
      if (
        ...
        if (config.requireExpects) {
          ...
          Logger.warn(
            "Counting each assert.step() for assert.expect() is changing in QUnit 3.0. Omit assert.expect() from tests that use assert.step(), or enable QUnit.config.countStepsAsOne to prepare for the upgrade. https://qunitjs.com/api/assert/expect/"
          );
        }
      }
      var actualCountForExpect = config.countStepsAsOne
      ...
      if (config.requireExpects && this.expected === null) {
        ...
        this.expected !== null &&
        this.expected !== actualCountForExpect &&
        this.stepsCount &&
        this.expected === this.assertions.length - this.stepsCount &&
        !config.countStepsAsOne
      ) {
        this.pushFailure(
          "Expected " +
          ...
          config.timeout = setTimeout$1(function () {
            ...
            if (!config._deprecated_timeout_shown) {
              ...
              Logger.warn(
                'Test "'.concat(
                  test.testName,
                  '" took longer than 3000ms, but no timeout was set. Set QUnit.config.testTimeout or call assert.timeout() to avoid a timeout in QUnit 3. https://qunitjs.com/api/config/testTimeout/'
                )
              );
            }
          }, 3000);
    ...
    return _createClass(ProcessingQueue, [
      ...
      },
      {
        ...
        value: function addToTaskQueue(tasksArray) {
          var _this$taskQueue;
          (_this$taskQueue = this.taskQueue).push.apply(
            _this$taskQueue,
            _toConsumableArray(tasksArray)
          );
        },
        ...
        value: function add(testTasksFunc, prioritize) {
          if (prioritize) {
            ...
            var index = Math.floor(
              this.unitSampler() *
                (config.queue.length - this.priorityCount + 1)
            );
